...
int main(int argc, char *argv[])  
{
    ...
    // Set up vaddr and cpid
    cproc.setCSR((uint64_t) hMem, 0);
    cproc.setCSR(cproc.getCpid(), 1);
    std::cout << "Host memory at: " << cproc.getCSR(0) << std::endl;
    std::cout << "Cpid: " << cproc.getCSR(1) << std::endl;
    ...
    auto benchmark_thr = [&]() {
        ...
        for(int i = 0; i < n_reps; i++)
            cproc.invoke({CoyoteOper::READ, (void*) hMem, size, false, false});

        while(cproc.checkCompleted(CoyoteOper::WRITE) != n_reps * n_runs) { if( stalled.load() ) throw std::runtime_error("Stalled, SIGINT caught"); }
    };
    ...
    auto benchmark_lat = [&]() {
        // Transfer the data
        for(int i = 0; i < n_reps; i++) {
                cproc.invoke({CoyoteOper::READ, (void*) hMem, (void*) hMem, size, size, true, false});
                while(cproc.checkCompleted(CoyoteOper::WRITE) != 1) { if( stalled.load() ) throw std::runtime_error("Stalled, SIGINT caught"); }                    
        }
    };
